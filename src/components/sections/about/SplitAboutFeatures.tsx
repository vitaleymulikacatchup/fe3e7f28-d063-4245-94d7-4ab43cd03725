          ...
          <div className={cls("relative w-full md:w-1/2 grid grid-cols-2 gap-6", featuresContainerClassName)}>
            {features.map((feature, index) => {
              ...
              return (
                <div
                  key={index}
                  className={cls("relative card rounded-theme-capped w-full flex flex-col md:flex-row md:items-center gap-3 p-8", isLastItemOdd && "col-span-2", featureClassName)}
                >
                  <div className="relative z-1 h-8 w-fit aspect-square rounded-theme primary-button flex items-center justify-center" >
                    <Icon
                      className={cls("h-45/100 w-45/100 text-background", featureIconClassName)}
                      aria-hidden="true"
                    />
                  </div>
                  <p className={cls("relative z-1 text-base leading-tight md:line-clamp-2", shouldUseLightText ? "text-background/75" : "text-foreground/75", featureTextClassName)}>
                    {feature.text}
                  </p>
                </div>
              );
