...
const MediaGridAbout = ({
    ...
    const mediaCard = (
        <div className={cls("card md:aspect-square rounded-theme-capped overflow-hidden p-6", mediaCardClassName)}>
            <div className={cls("relative z-1 grid h-full gap-6", getGridLayout(), mediaGridClassName)}>
                {items.map((item, index) => (
                    <div key={index} className={cls("relative h-30 md:h-auto overflow-hidden", getItemClassName(index))}>
                        <MediaContent
    ...
    return (
        <section
            ...
            className={cls("relative py-20 bg-red-100", useInvertedBackground === "invertCard" ? "w-content-width-expanded mx-auto rounded-theme-capped bg-foreground" : "w-full", useInvertedBackground === "invertDefault" && "bg-foreground", className)}
        >
